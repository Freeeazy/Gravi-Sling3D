...
Shader "ProceduralSpace"
{
	...
	{
		_Seed("Seed", Float) = 1
		_PixelGrid("Pixel Grid (cells per unit)", Range(1, 512)) = 64
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i ) : SV_Target
			{
				...
				// quantize the lookup position for ALL procedural sampling
				float pg = max(_PixelGrid, 1.0);
				float3 P = floor(Position79 * pg) / pg;
				float simplePerlin3D42 = snoise( ( P * 1.0 ) );
				float simplePerlin3D44 = snoise( ( P * 5.0 ) );
				float lerpResult29 = lerp( -0.6 , -0.9 , ( 1.0 - _NumStars ));
				float simplePerlin3D19 = snoise( ( P * 80.0 ) );
				float smoothstepResult25 = smoothstep( lerpResult29 , -1.0 , simplePerlin3D19);
				float simplePerlin2D36 = snoise( ( P * 2.0 ).xy );
				...
				float simplePerlin3D56  = snoise( ( P * 0.4 ) );
				...
				float simplePerlin3D105 = snoise( ( P * 2.0 ) );
				float lerpResult231 = lerp( 0.5 , 1.0 , _Nebular1Strength);
				float simplePerlin3D188 = snoise( ( P * 5.0 ) );
				float simplePerlin3D192 = snoise( ( P * 10.0 ) );
				float simplePerlin3D204 = snoise( ( P * 150.0 ) );
				...
				float simplePerlin3D171 = snoise( ( ( P + 100.0 ) * 0.9 ) );
				float simplePerlin3D172 = snoise( ( P * 0.9 ) );
